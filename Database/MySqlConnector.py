...
def load_group(group_website, load_users_data=True, load_giveaway_data=True, fetch_not_started_giveaways=False, limit_by_time=False, starts_after_str=None, ends_before_str=None, ends_after_str=None):
    ...
    if load_users_data and group_users_data:
        ...
        cursor.execute('SELECT * FROM Users WHERE UserName in (' + parse_list(list(group_users.keys())) + ')')
    ...
    if load_giveaway_data and group_giveaways_data:
        ...
        cursor.execute('SELECT * FROM Giveaways WHERE GiveawayID in (' +  parse_list(list(giveaways_by_id.keys())) + ')')
        data = cursor.fetchall()
        for row in data:
            ...
            group_giveaways[giveaway_link].game_name = str(row[3])
...
def delete_users(users):
    ...
    cursor.execute('DELETE FROM Users WHERE UserName IN(' + parse_list(list(map(lambda x: x.user_name or '', users))) + ')')
    connection.commit()  # you need to call commit() method to save your changes to the database
...
def remove_games(games):
    ...
    cursor.execute('DELETE FROM Games WHERE Name IN (' + parse_list(list(map(lambda x: x.game_name or '', games))) + ')')
    connection.commit()  # you need to call commit() method to save your changes to the database
...
def overwrite_bundled_games(bundled_games):
    start_time = time.time()
    connection = pymysql.connect(host=host, port=port, user=user, passwd=password, db=db_schema, charset='utf8')
    cursor = connection.cursor()
    ...
    for bundled_game in bundled_games:
        bundled_games_data.append((bundled_game.app_id, bundled_game.package_id, bundled_game.game_name, bundled_game.was_bundled, bundled_game.was_free))

    cursor.execute("TRUNCATE TABLE BundledGames")
    cursor.executemany("INSERT INTO BundledGames (AppId,PackageId,GameName,WasBundled,WasFree) VALUES (%s, %s, %s, %s, %s)", bundled_games_data)
    connection.commit()  # you need to call commit() method to save your changes to the database
