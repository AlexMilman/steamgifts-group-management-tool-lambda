...
def check_multiple_wins(user):
    return WebUtils.get_page_content(get_multiple_wins_url(user)).find(' has no multiple wins for the same game!') == -1


def check_nonactivated(user):
    nonactivated_page_content = WebUtils.get_page_content(get_nonactivated_url(user))
    return nonactivated_page_content.find(' has activated all his/her games!') == -1 and nonactivated_page_content.find(
        'Games wins not activated') != -1


def get_nonactivated_url(user):
    return SGToolsConsts.SGTOOLS_CHECK_NONACTIVATED_LINK + user


def get_multiple_wins_url(user):
    return SGToolsConsts.SGTOOLS_CHECK_MULTIPLE_WINS_LINK + user
